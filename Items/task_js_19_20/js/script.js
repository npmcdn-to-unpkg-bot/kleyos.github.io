...
var arrayConcat = [];
_(dataJSON).forEach(function(value) {
...
var arrayNames = [];
_(dataJSON).forEach(function(value) {
  arrayNames = _.concat(arrayNames, {'name':value.name, 'friends':_.size(value.friends)});
});

var arraySort = _.sortBy(arrayNames,function(obj){
  return obj.friends;
});
console.log('Массив имен отсортированных по количеству friends',_.map(arraySort, 'name'));


// 3. Массив всех друзей всех пользователей,
// не должно быть повторяющихся людей
var arrayNames = [];
_(dataJSON).forEach(function(value) {
  arrayNames = _.concat(arrayNames, _.map(value.friends, 'name'));
});
console.log('Массив всех друзей всех пользователей: ',_.union(arrayNames));
